...
1. `state/current.md` — QUILL_STATE（プロジェクトの現在状態）
...
2. 生成した QUILL_STATE を state/current.md に保存する
3. キャラクターファイルを characters/main_cast/ に作成する
4. git commit（"quill: [TITLE] プロジェクト開始"）
```
...
3. 登場するキャラのファイルを読む（全員ではなく今回必要な人だけ）
4. engines/quill_engine.md の PHASE_2（シーン執筆）に従う
5. scenes/scene_N.md に保存する
6. engines/eval_engine.md に従い評価を出力する
7. state/current.md の PROGRESS・MEMO を更新する
...
| 3 | `state/current.md` | セッション開始時・シーン執筆前 |
| 4 | 関連する `characters/main_cast/` | 今回登場するキャラのみ |
...
- `state/current.md` の手動編集をユーザーに提案しない（自動更新のみ）
