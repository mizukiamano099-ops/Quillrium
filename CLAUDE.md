...
```
0. （任意）engines/profiling.md の PHASE_0 を実行する
   → AUTHOR_PROFILE を state/current.md に追記する

   ── AUTHOR_PROFILE の引き継ぎルール ──
   既存の Story/{他TITLE}/state/current.md に AUTHOR_PROFILE が設定済みの場合：
   「前回のプロファイル（{要約}）を引き継ぎますか？」と確認を取る
   → 「はい」：そのまま新しい state/current.md にコピーする
   → 「いいえ」：PHASE_0 を再実行するか、Exploratory のままにするか確認する
   → プロファイリング未実施の場合は Exploratory をデフォルトとして動作する
...
│   ├── kinsoku.md              ← 禁則処理ルール
│   ├── profiling.md            ← 追加（Layer 2）
│   ├── yorisoi.md              ← 追加（Layer 3）
│   └── shruti.md               ← 追加（Shruti連携）
│
├── characters/
...
| 8 | `engines/profiling.md` | 新規プロジェクト・プロファイリング時 |
| 9 | `engines/yorisoi.md` | セッション開始時（AUTHOR_PROFILE読み込み後）|
|10 | `engines/shruti.md` | ユーザーが楽曲化を求めたとき |
---
...
- プロファイリングを強制しない（PHASE_0 は任意）
- AUTHOR_PROFILE がない場合は Exploratory をデフォルトとして動作する
